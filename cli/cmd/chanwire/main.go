...
import (
	...
	"github.com/juex-ai/chanwire/cli/internal/version"
)

// Build-time variables injected via -ldflags.
// The ldflags target this package's main.version / main.commit.
var (
...
func agentRegisterCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Printf("registered: agent_name=%s\n", resp.AgentName)
			return nil
		},
	}
...
func connectCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return wsc.ConnectWithReset(ctx, os.Stdout)
		},
	}
}
...
// requireToken loads agent.json and returns ErrNotRegistered (printed to
...
func requireToken() (*store.AgentInfo, error) {
	...
	if err != nil {
		if errors.Is(err, store.ErrNotRegistered) {
			fmt.Fprintln(os.Stderr, store.ErrNotRegistered.Error())
			os.Exit(1)
		}
		return nil, err
	}
